...
function CreditHeroPreview() {
  return (
    <div className="hero-simulation-card relative overflow-hidden rounded-[16px] border border-border bg-white p-7 sm:p-8">
      <div className="absolute inset-x-0 top-0 h-20 bg-[linear-gradient(180deg,rgba(37,99,235,0.05),transparent)]" />
      ...
      <div className="relative space-y-6">
          ...
          <div className="rounded-[14px] border border-border bg-[#F8FAFC] p-4">
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-sm text-slate-600">Usamos isso apenas para mostrar um caminho mais coerente.</p>
              </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {['Sem pressão', 'Sem custo para começar'].map((item) => (
            <div key={item} className="flex items-center gap-2 rounded-[14px] border border-border bg-[#F8FAFC] px-4 py-3 text-sm text-slate-700">
        ...
        <button
          type="button"
          className="flex h-12 w-full items-center justify-center gap-2 rounded-[12px] bg-primary text-sm font-medium text-white transition-all duration-300 hover:scale-[1.01] hover:bg-primary-hover"
        >
          Continuar
