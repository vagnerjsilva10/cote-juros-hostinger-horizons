...
export default function CreditasExtraForm({ lead, quizAnswers, recommendation, onStatus }) {
  ...
  const submit = async (event) => {
    ...
    if (leadResult?.mode === 'fallback') {
      setStatus('fallback');
      setMessage('Não conseguimos consultar a parceira agora. Seus dados foram salvos para continuidade.');
      return;
    }
    ...
    if (leadResult?.ok) {
      ...
      setMessage(finalStatus === 'not_eligible'
        ...
        : 'Enviado para análise. Condições dependem da avaliação da Creditas.');
      return;
    }

    setStatus('error');
    setMessage(leadResult?.message || 'Não conseguimos consultar a parceira agora. Seus dados foram salvos para continuidade.');
  };
  ...
  return (
    <form className="creditas-extra-form" onSubmit={submit}>
      <div className="mb-4 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm leading-6 text-white/65">
            Complete apenas os dados necessários para consultar opções com garantia, sujeito à análise.
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <Input value={form.assetValue} onChange={(event) => update('assetValue', event.target.value)} placeholder="Valor estimado do bem" className="bg-white text-slate-950 sm:col-span-2" />
      </div>
      ...
      {message ? (
        <div className="mt-3 rounded-[16px] border border-white/10 bg-white/[0.04] p-3 text-sm leading-6 text-white/72">
          {message}
          {requiredFields.length ? <div className="mt-1 text-white/50">Campos: {requiredFields.join(', ')}</div> : null}
        </div>
      ) : null}
