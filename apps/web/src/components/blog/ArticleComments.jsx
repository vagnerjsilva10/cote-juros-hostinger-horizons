...
const COMMENTS_STORAGE_KEY = 'cj.blog.comments.v1';
...
function ArticleComments({ articleSlug }) {
  ...
  useEffect(() => {
    ...
    try {
      window.localStorage.setItem(COMMENTS_STORAGE_KEY, JSON.stringify(submitted));
    } catch {
    ...
    <section className="rounded-[20px] border border-border bg-white p-6 md:p-8">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Os comentários ficam salvos neste navegador enquanto a moderação em backend é preparada.</p>
        </div>
      </div>
      ...
      <div className="mt-8 space-y-4">
        {comments.length ? (
          ...
          <div className="rounded-[16px] border border-dashed border-border bg-background-secondary p-5 text-sm text-muted-foreground">
            Nenhum comentário ainda. Quando alguém comentar por aqui, a mensagem vai continuar visível neste dispositivo mesmo após recarregar a página.
          </div>
        )}
      </div>
    </section>
