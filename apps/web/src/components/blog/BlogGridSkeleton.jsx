...
function BlogGridSkeleton({ items = 6 }) {
  return (
    <div className="blog-grid-skeleton grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
      {Array.from({ length: items }).map((_, index) => (
        <div key={index} className="blog-grid-skeleton-card overflow-hidden rounded-[24px] border border-border bg-white">
