...
function FinanciamentoPage() {
  const handleSimulate = (bank) => {
    toast.success(`Iniciando simulação com ${bank}...`);
  };

  const renderCards = (typesArray) => {
    const data = financingData.filter(f => typesArray.includes(f.type));
          ...
          <Card key={fin.id} className="card-premium overflow-hidden bg-white border-slate-200">
            <CardContent className="p-0">
              <div className="p-6 md:p-8 border-b border-slate-100 flex justify-between items-start bg-slate-50">
                <div>
                  <Badge variant="outline" className="mb-3 bg-white">{fin.type}</Badge>
              ...
              <div className="p-6 md:p-8 space-y-6">
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-muted-foreground mb-1 flex items-center gap-1"><Calendar className="w-4 h-4"/> Prazo Máx</p>
                  ...
                  <div className="col-span-2 bg-slate-50 rounded-lg p-4 border border-slate-100">
                    <p className="text-sm font-semibold text-muted-foreground mb-1">Condição de Entrada</p>
                    <p className="text-base font-bold text-foreground">A partir de {fin.minDownPayment}% do valor do bem</p>
                  </div>
                </div>

                <Button className="w-full h-14 text-lg font-bold gradient-fintech-hover border-0 shadow-[var(--shadow-sm)]" onClick={() => handleSimulate(fin.bankName)}>
    ...
    <>
      ...
      <div className="bg-foreground text-background py-16 lg:py-20 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center max-w-3xl">
          ...
          <p className="text-xl text-slate-300 font-medium">Compare as taxas de financiamento imobiliário e de veículos nos maiores bancos do país.</p>
        </div>
      </div>
          ...
          <TabsContent value="imobiliario" className="mt-4 animate-in fade-in-50 slide-in-from-bottom-4 duration-500">
            <div className="bg-purple-50 border border-purple-100 rounded-[var(--radius-lg)] p-6 mb-8 mt-8 flex items-start gap-4">
              ...
              <p className="text-purple-900 font-medium leading-relaxed">
                Para o financiamento imobiliário, você pode usar seu FGTS como entrada se o imóvel se enquadrar nas regras do Sistema Financeiro de Habitação (SFH).
              </p>
            </div>
            {renderCards(['Imobiliário', 'Refinanciamento'])}
          </TabsContent>
        </Tabs>
      </div>
    </>
