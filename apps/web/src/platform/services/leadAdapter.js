...
const mapLeadToSimulation = (payload = {}) => {
  ...
  return {
    ...
    requestedAmount: parseMoney(answers.requestedAmount ?? answers.amount ?? answers.valor ?? payload.requestedAmount),
    income: parseMoney(answers.monthlyIncome ?? answers.income ?? answers.renda ?? payload.income),
    scoreRange: String(payload.score ?? recommendation.score ?? ''),
    employmentStatus: answers.occupation || answers.workType || answers.tipoTrabalho || answers.employmentStatus || payload.employmentStatus,
    hasRestriction: ['yes', 'sim', true].includes(answers.hasNegativeStatus ?? answers.hasRestriction ?? answers.negativado ?? payload.hasRestriction),
    fullName: payload.name || payload.fullName || answers.contactName,
    phone: normalizePhone(payload.phone || payload.whatsapp || answers.whatsapp),
